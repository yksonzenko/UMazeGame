#include "PinkPinky.hpp"

namespace GameEngine
{
	PinkPinky::PinkPinky(Engine::Scene* scene) : Actor(scene)
	{
		assignAllComponents();
		fillComponents();
	}

	void PinkPinky::assignAllComponents()
	{
	...
	void PinkPinky::fillComponents()
	{
		auto transform = entity_.component<Transform>();
		transform->position = { 15, 11 };
		transform->spawnPos = { 15, 11 };
		...
		ai->moveDistance = 4; // distance from target
	...
	void PinkPinky::setAIMode(AIBehaviourMode::Mode mode)
	{
		auto transform = entity_.component<Transform>();
		auto anim = entity_.component<Animation>();
		auto ai = entity_.component<AIMovement>();
